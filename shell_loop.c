 ...
 *
 * Return: 0 for a successful prog run otherwise 1 or error code
 */
 ...
 *
 * Return: -1 if buitin nt found, o if executed successfully,
...
/**
 * fork_cmd - Will fork an executable trhead to run cmd
 * @info: The parameter and return info struct
 *
 * Return: void
 */
void fork_cmd(info_t *info)
{
	...
	else
	{
		...
		if (WIFEXITED(info->status))
		{
			...
			if (info->status == 126)
				print_error(info, "Premission denied\n");
		}
	}
}
